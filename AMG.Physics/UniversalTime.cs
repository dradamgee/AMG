...
namespace AMG.Physics
{
    public class UniversalTime {
        private readonly IEnumerable<TimeDependentActionable> _actions;
        private readonly CancellationToken _cancelToken;
        private const int _interval = 25;
        private const double _intervalInSecounds = 0.025d;
        private Task time;

        public UniversalTime(IEnumerable<TimeDependentActionable> actions, CancellationToken cancelToken)
        {
            _actions = actions;
        ...
        private void Work()
        {
            while (true)
            {
                ...
                foreach (TimeDependentActionable timeDependentAction in _actions) {
